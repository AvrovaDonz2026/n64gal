...
- [x] x64 真实后端优先级单测：已新增 `test_backend_priority`，直接覆盖 `force scalar / force avx2 / SIMD auto` 三条关键选择路径。
...
1. `x64 perf host evidence` 继续沉淀
2. `row-level 常量前折叠`（如仍有必要）
3. `avx2_backend.c` 拆文件 + Clang/MSVC 兼容补证
4. `full-span textured kernel` 长期跟踪
