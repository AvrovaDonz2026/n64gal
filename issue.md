...
4. Validation：`test_dynamic_resolution` 覆盖 controller 逻辑，`test_runtime_dynamic_resolution` 覆盖真实 runtime 降档路径（`scalar + S3 + 1200x1600 + hold-end`）；`run_cc_suite` 已纳入这两条测试。
...
- [x] Dirty-Tile / Dynamic Resolution 都有稳定的 on/off compare artifact
- [ ] `S0 p95 <= 12.0ms`
- [ ] `S1 p95 <= 14.0ms`
- [ ] `S2 p95 <= 15.0ms`
- [ ] `S3 p95 <= 16.2ms`
...
- [x] `linux-x64` perf artifact 已同时固化 `scalar -> avx2` threshold report、`avx2 dirty off/on` compare summary，与 `scalar dynres off/on` compare summary
