...
5. Validation：当前 `unit dirty planner`、`runtime_api`、`preview_protocol`、`test_renderer_dirty_submit`、`runtime_golden` dirty-on，以及 `qemu-rvv` 下的 `test_renderer_dirty_submit_rvv` 已覆盖最小链路；下一步继续补 perf compare 的 `dirty-tile on/off` 对照。
