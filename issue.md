...
- [ ] NEON row-palette gather/apply 继续降开销，并按 native arm64 artifact 复核 heuristic（当前 alpha path 已预打包 `full/RB/G` palette，opaque path 也已补整行缓存复用；剩余重点转向更低开销的 gather/apply 本体）
