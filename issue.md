...
12. `ISSUE-008` 已把 Runtime `VNRenderOp[]` LRU 命令缓存接入主线：`VNRunConfig.perf_flags` / `VNRunResult.perf_flags_effective|op_cache_hits|op_cache_misses` 已公开，CLI 已支持 `--perf-op-cache=<on|off>`，命中路径会按当前帧回写 `SPRITE/FADE` 动态字段以避免伪动画导致全 miss，`test_runtime_api` 已覆盖命中/关闭两条路径。
...
9. `ISSUE-008`（进行中）：性能门限文件、门限校验脚本、`linux-x64` compare gate、`qemu-rvv` revision compare soft gate，以及 Runtime `VNRenderOp[]` LRU 命令缓存已落地；剩余 P0 本体为 state-hash / dirty-tile / dynamic resolution
...
4. `ISSUE-008` 第二阶段：在已有 perf threshold gate 与 op cache 之上继续做 state-hash / dirty-tile / dynamic resolution 本体优化。
...
4. `ISSUE-008`：在已落地 perf threshold gate 的基础上继续做 P0 四件套优化本体。
...
4. `PR-M3-006`：P0 四件套中的首个可量化优化（优先 state-hash 或 dirty-tile）（`ISSUE-008`）
...
先建立性能回归门限文件与 CI 比对开关矩阵，再继续落实白皮书 P0 优化：静态帧短路、Dirty-Tile、命令缓存、动态分辨率；在正确性前提下尽可能挖掘可获得性能收益。
