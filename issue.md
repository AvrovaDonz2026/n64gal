...
7. P2 兼容：Linux x64 现已新增 `CC=clang` 的 native suite + `matched backend=avx2` 补证，`windows-x64` 也已新增 `MSVC Debug compile-only` 证据；后续继续评估 MSVC `/arch:AVX2` 在“无 AVX2 机器安全回退”上的覆盖策略。
