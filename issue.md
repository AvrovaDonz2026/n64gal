...
5. `ISSUE-011` 收口：RVV qemu perf_compare 证据已补齐（`docs/perf-rvv-2026-03-06.md`），并已接入可重复 artifact 流程；现阶段优先继续做 qemu 侧门限与一致性收口。
...
- [ ] `qemu-rvv` revision compare gate 接线（默认可选，待观察噪声后决定是否转阻塞）
- [ ] 静态帧短路（state hash）
- [ ] Dirty-Tile 增量渲染
- [ ] 命令缓存（LRU）
