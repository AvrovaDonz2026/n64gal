...
15. `ISSUE-008` 第一段代码已落地：新增 `src/frontend/dirty_tiles.c/.h` 内部 planner、`VN_RUNTIME_PERF_DIRTY_TILE`、`VNRunResult` dirty 统计字段、preview JSON 回显、`test_dirty_tiles` 与 `test_runtime_api` 覆盖，以及 CLI `--perf-dirty-tile=<on|off>`；当前该路径只产出 dirty plan/统计，尚未接入 backend dirty submit。
...
- [x] Dirty-Tile runtime flag / result / preview / CLI 接线（observe-only）
- [x] Dirty-Tile 内部 planner 与统计（`src/frontend/dirty_tiles.c/.h`，全帧提交不变）
- [ ] Dirty-Tile 增量渲染
- [x] 命令缓存（LRU）
...
2. Frontend：dirty planner 已落地，当前比较 `last_presented_ops[]` 与当前帧最终 `VNRenderOp[]`，输出 tile 计数与 clip rect 列表，但还未改变 renderer 提交路径。
3. Renderer/API：下一步新增 `renderer_submit_dirty(...)` 与 backend `submit_ops_dirty(...)` 契约，先保证“未实现时自动回退整帧路径”。
4. Backend：先做 `scalar` dirty submit 作为 golden 基线，再逐步补 `avx2` / `neon` / `rvv`。
5. Validation：当前 `unit dirty planner`、`runtime_api`、`preview_protocol` 已补最小覆盖；下一步继续补 `backend_consistency`、`runtime_golden` 与 perf compare 的 `dirty-tile on/off` 对照。
