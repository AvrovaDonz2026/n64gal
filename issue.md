...
1. Runtime：新增 `VN_RUNTIME_PERF_DIRTY_TILE`、trace/summary/result 统计字段与 `--perf-dirty-tile=<on|off>` 开关，默认先保持 `off`。
2. Frontend：新增 dirty planner，比较 `last_presented_ops[]` 与当前帧最终 `VNRenderOp[]`，输出 tile 计数与 clip rect 列表。
3. Renderer/API：新增 `renderer_submit_dirty(...)` 与 backend `submit_ops_dirty(...)` 草案接口，先保证“未实现时自动回退整帧路径”。
4. Backend：先做 `scalar` dirty submit 作为 golden 基线，再逐步补 `avx2` / `neon` / `rvv`。
5. Validation：补 `unit dirty planner`、`runtime_api`、`backend_consistency`、`runtime_golden` 与 perf compare 的 `dirty-tile on/off` 对照。
