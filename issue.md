...
6. P2 结构：按职责拆分 `src/backend/avx2/avx2_backend.c`，至少拆成 `fill_fade`、`textured`、`dirty/debug` 三块，降低 textured 热路径改动的回归半径。
7. P2 兼容：补一条 Clang x64 AVX2 编译实证，并评估 MSVC `/arch:AVX2` 在“无 AVX2 机器安全回退”上的覆盖策略，避免只在有 AVX2 的 GitHub runner 上成立。
...
- [ ] row-palette build 降开销：减少 256 项 palette build 的逐项 `sample_combine` 成本，优先考虑向量批量构建或更多 row-level 前折叠。
- [ ] row-palette heuristic 策略化：把 `vis_w > 256` 从硬编码常量升级成可验证的策略函数，并用 perf 证据解释当前阈值依据。
...
- [ ] x64 真实后端优先级单测：对 `avx2 -> neon -> rvv -> scalar` 的真实选择顺序补直接断言，不只靠高层行为间接覆盖。

#### Perf/CI 主线

- [ ] `S10` 独立门限：给 `linux-x64-scalar-avx2-smoke` 与 `windows-x64-scalar-avx2-smoke` 增加 `S10` 的单独 ceiling 或 gain floor，不再只作为 coverage sample。
- [ ] kernel threshold：把 `kernel scalar -> avx2` 的 `sprite_full_opaque` / `sprite_full_alpha180` 纳入 threshold profile，先以 `soft/report-only` 落地，再视稳定性决定是否升级成阻塞门限。
- [ ] `windows-x64 dirty jitter` 长窗口任务：在保留短窗口 smoke gate 的同时，单独补 `10s/2s + repeat=5` 级别的 report-only repeat 任务，持续观察 `S3` 噪声。
- [ ] x64 perf host evidence 继续沉淀：保持 compare artifact 输出 `host_cpu`，并在文档里持续记录 Linux/Windows runner CPU 差异对 AVX2 收益形状的影响。

### 近期执行顺序（建议）

1. `avx2 dirty golden` + `fast-path parity 单测`
2. `S10` 独立门限 + `kernel threshold(report-only)`
3. `row-palette build 降开销` + `row-palette heuristic 策略化`
4. `windows-x64 dirty jitter` 长窗口任务
5. `avx2_backend.c` 拆文件 + Clang/MSVC 兼容补证
...
- [ ] NEON row-palette gather/apply 继续降开销，并按 native arm64 artifact 复核 heuristic（alpha blend 已收口到 packed-channel chunk helper，剩余重点转向 gather/apply 本身）
- [ ] NEON textured row 继续减少 `u_lut` lane-load 与 chunk pack 开销，评估更低指令数或更宽 chunk 版本
- [x] aarch64 交叉编译通过
