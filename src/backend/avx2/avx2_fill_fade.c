...
static void vn_avx2_blend_uniform_u32_asm(vn_u32* dst, vn_u32 count, vn_u32 src, vn_u8 alpha) {
    ...
    vn_u32 i;
    vn_u32* ptr;
    const vn_u32 src_rb = src & 0x00FF00FFu;
    const vn_u32 src_g = src & 0x0000FF00u;
    const vn_u32 alpha_u32 = (vn_u32)alpha;
    const vn_u32 inv_u32 = 255u - (vn_u32)alpha;
    const vn_u32 mask_rb = 0x00FF00FFu;
    const vn_u32 mask_g = 0x0000FF00u;
    const vn_u32 bias_rb = 0x007F007Fu;
    const vn_u32 bias_g = 0x00007F00u;
    const vn_u32 one_rb = 0x00010001u;
    const vn_u32 one_g = 0x00000100u;
    const vn_u32 alpha_mask = 0xFF000000u;

    if (dst == (vn_u32*)0 || count == 0u || alpha == 0u) {
        return;
    }
    if (alpha >= 255u) {
        vn_avx2_fill_u32_asm(dst, count, src);
        return;
    }
    ...
    if (bulk_pixels != 0u) {
        remaining_pixels = bulk_pixels;
        __asm__ __volatile__(
            "vbroadcastss %[src_rb], %%ymm0\n\t"
            "vbroadcastss %[src_g], %%ymm1\n\t"
            "vbroadcastss %[alpha], %%ymm2\n\t"
            "vbroadcastss %[inv], %%ymm3\n\t"
            "vbroadcastss %[mask_rb], %%ymm4\n\t"
            "vbroadcastss %[mask_g], %%ymm5\n\t"
            "vbroadcastss %[bias_rb], %%ymm6\n\t"
            "vbroadcastss %[bias_g], %%ymm7\n\t"
            "vbroadcastss %[one_rb], %%ymm12\n\t"
            "vbroadcastss %[one_g], %%ymm13\n\t"
            "vbroadcastss %[alpha_mask], %%ymm14\n\t"
            "1:\n\t"
            "vmovdqu (%[ptr]), %%ymm8\n\t"
            "vpand %%ymm4, %%ymm8, %%ymm9\n\t"
            "vpand %%ymm5, %%ymm8, %%ymm10\n\t"
            "vpmulld %%ymm2, %%ymm0, %%ymm11\n\t"
            "vpmulld %%ymm3, %%ymm9, %%ymm15\n\t"
            "vpaddd %%ymm15, %%ymm11, %%ymm11\n\t"
            "vpaddd %%ymm6, %%ymm11, %%ymm11\n\t"
            "vpsrld $8, %%ymm11, %%ymm15\n\t"
            "vpand %%ymm4, %%ymm15, %%ymm15\n\t"
            "vpaddd %%ymm12, %%ymm15, %%ymm15\n\t"
            "vpaddd %%ymm15, %%ymm11, %%ymm11\n\t"
            "vpsrld $8, %%ymm11, %%ymm11\n\t"
            "vpand %%ymm4, %%ymm11, %%ymm11\n\t"
            "vpmulld %%ymm2, %%ymm1, %%ymm15\n\t"
            "vpmulld %%ymm3, %%ymm10, %%ymm9\n\t"
            "vpaddd %%ymm9, %%ymm15, %%ymm15\n\t"
            "vpaddd %%ymm7, %%ymm15, %%ymm15\n\t"
            "vpsrld $8, %%ymm15, %%ymm9\n\t"
            "vpand %%ymm5, %%ymm9, %%ymm9\n\t"
            "vpaddd %%ymm13, %%ymm9, %%ymm9\n\t"
            "vpaddd %%ymm9, %%ymm15, %%ymm15\n\t"
            "vpsrld $8, %%ymm15, %%ymm15\n\t"
