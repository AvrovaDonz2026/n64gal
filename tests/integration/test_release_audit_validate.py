...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="n64gal_release_audit_") as temp_dir:
        ...
        publish_map.write_text(
            '{"tag":"v0.1.0-alpha","release_url":"https://github.com/AvrovaDonz2026/n64gal/releases/tag/v0.1.0-alpha","asset":{"path":"assets/demo/demo.vnpak","sha256":"deadbeef","bytes":1}}\n',
            encoding="utf-8",
        )

        rc, out, err = run_case(["--allow-dirty", "--bundle-manifest", str(bundle_manifest), "--publish-map", str(publish_map), str(temp_root)])
        if rc != 0:
        ...
        publish_map.write_text('{"tag":"broken","release_url":"bad","asset":{"path":"broken","sha256":"","bytes":0}}\n', encoding="utf-8")
