...
int main(void) {
    ...
    if (vm_fade_serial(&vm) != 1u) {
        (void)fprintf(stderr, "fade serial should be 1 after first fade\n");
        return 1;
    }
    ...
    if (vm_fade_serial(&vm) != 0u) {
        (void)fprintf(stderr, "second choice path should not execute fade\n");
        return 1;
    }
