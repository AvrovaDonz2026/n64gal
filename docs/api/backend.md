...
4. `rvv`：已接入最小可运行链路，`fill`、不透明矩形填充、统一颜色半透明 `fade/fill`，以及 `SPRITE/TEXT` 的 `combine + alpha` 写回路径使用 RVV 向量写入，目标架构外返回 `VN_E_UNSUPPORTED`。`riscv64` 交叉构建、`qemu-user` 冒烟与 `scalar vs rvv` CRC 对照已在本地验证。
...
7. `rvv` 当前保持 `tex sample` 标量一致性路径，但会对整行 `combine` 与 alpha 写回做批量向量处理，优先压缩热路径尾部成本。
