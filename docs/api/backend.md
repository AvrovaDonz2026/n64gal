...
3. `avx2_asm`：实验性 force-only 变体，当前与 `avx2` 共享同一套 framebuffer / textured / dirty-submit 主路径，但只在 GNU x64 且 ASM kernel 可用时允许初始化；否则强制请求也会按现有 forced backend 语义回退到 `scalar`。当前阶段它已覆盖 `clear/fill(alpha=255)` 与 `uniform fade/blend` 热路径，本地最新 `kernel avx2 -> avx2_asm` 样本里 `clear_full p95 0.797ms -> 0.105ms`、`fade_full_alpha160 p95 4.938ms -> 1.095ms`，而 textured 类 kernel 仍大体持平，因此它暂时仍是“高风险、高收益但覆盖面集中在 fill/fade”的实验后端。auto 选择链故意不纳入 `avx2_asm`。
