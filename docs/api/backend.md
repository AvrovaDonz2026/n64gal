...
3. `avx2_asm`：实验性 force-only 变体，当前与 `avx2` 共享同一套 framebuffer / textured / dirty-submit 主路径，但只在 GNU x64 且 ASM fill 可用时允许初始化；否则强制请求也会按现有 forced backend 语义回退到 `scalar`。当前阶段它只替换 `clear/fill(alpha=255)` 热路径，本地最新 `kernel avx2 -> avx2_asm` 样本里 `clear_full p95 0.797ms -> 0.105ms`，而其它非 fill kernel 基本持平，因此它暂时仍是“高风险、高收益但覆盖面很窄”的实验后端。auto 选择链故意不纳入 `avx2_asm`。
