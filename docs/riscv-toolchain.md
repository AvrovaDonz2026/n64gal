...
| `R2` | qemu-rvv | 在 `qemu-user` 下验证 `rvv` 二进制能实际跑到 `rvv` | 阻塞 |
...
3. `.github/workflows/ci-matrix.yml` 已接入 `linux-riscv64-qemu-rvv` 阻塞 job；截至 2026-03-06，最近 12 轮 workflow 该 job 连续 `success`。
4. `.github/workflows/riscv-perf-report.yml` 已接入 `linux-riscv64-qemu-rvv-perf-report`；首次 `workflow_dispatch` run `22766736383` 已 `success`，artifact 与 step summary 均已验证。
5. 当前没有可重复使用的 `native-riscv64/RVV` 设备，因此 `R3` 暂列外部阻塞项，不放在普通 PR 阶段，也不作为当前日常开发的最高优先级。
...
| `linux-riscv64-qemu-scalar` | `scripts/ci/run_riscv64_qemu_suite.sh --skip-rvv` + `suite-linux-riscv64-qemu-scalar` artifact | 阻塞 |
| `linux-riscv64-qemu-rvv` | `scripts/ci/run_riscv64_qemu_suite.sh --require-rvv` + `suite-linux-riscv64-qemu-rvv` artifact | 阻塞 |
| `linux-riscv64-qemu-rvv-perf-report` | `.github/workflows/riscv-perf-report.yml` + qemu RVV revision perf markdown artifact | 夜间/手动 |
...
执行后会在 `build_ci_riscv64/` 下生成：

1. `ci_logs/*.log`
2. `ci_suite_summary.md`
3. `golden_artifacts/`（如 `test_runtime_golden` 输出了 diff/summary）
...
`linux-riscv64-qemu-rvv` 已在 2026-03-06 提升为阻塞项，依据是：

1. `rvv fill/blend/tex/combine` 核心热路径已基本齐备。
2. 最近 12 轮 workflow 中，`linux-riscv64-qemu-rvv` 连续 `success`。
3. `test_backend_consistency_riscv64` 与 `test_runtime_golden` 的语义对照链已接入 `qemu-rvv` 路线。
4. `rvv` 初始化失败回退到 `scalar` 的行为仍由 `renderer_fallback` / runtime 冒烟链单独覆盖。

## 8. 现阶段结论

在当前仓库状态下，最合理的执行顺序不是“直接上原生 perf”，而是：

1. 把 `R0 + R1 + R2` 固化到主线 CI，并补齐 `qemu` 侧 golden/perf artifact；当前已落地 qemu suite log/summary artifact，后续继续收口 threshold gate 与 Windows/原生证据链。
2. 在没有原生设备的阶段，按 `qemu-first` 策略推进，不把 `R3` 缺口混入日常开发阻塞。
3. 待原生 riscv64 runner 或开发板就绪后，再把 `R3` 接上。
4. 性能结论仍只接受原生 riscv64 Linux 采样，不接受 `qemu-user` 结果替代。
