...
3. `avx2_asm`：实验性 force-only 变体，仅在 GNU x64 下允许初始化，并在当前阶段只替换 `clear/fill(alpha=255)` 的 x64 ASM 热路径；其它平台或 toolchain 上强制请求会回退 `scalar`。这条线现已接入 `test_renderer_fallback`、`test_backend_priority`、`test_renderer_dirty_submit`、`test_backend_consistency` 与 `test_runtime_golden` 的最小覆盖，本地 `run_cc_suite` 已复核通过。最新一轮 `kernel avx2 -> avx2_asm` 样本里，`clear_full p95 0.797ms -> 0.105ms`，而非 fill 热点基本持平，说明当前主要收益集中在 fill/clear。
