...
2. `avx2`：可运行实现已接入（`CLEAR/SPRITE/TEXT/FADE` + `tex/combine` 采样），CPU 不支持时自动回退 `scalar`；当前阶段判断为“已稳定，转维护优化”，其中 x64 CI 的 `matched backend=avx2` 显式断言、perf artifact 的 `requested_backend/actual_backend` 记账、`avx2_dirty` golden 对照，以及 `256/257` 边界 fast-path parity 单测都已经补齐；剩余重点转向 row-palette build 去标量化、`S10`/kernel 级 perf gate 与 `windows-x64 dirty S3` 噪声收敛。
