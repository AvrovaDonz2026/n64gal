...
1. 已完成：
   ...
   - Runtime 版本协商与最小 persistence draft：`vn_runtime_query_build_info(...)`、`capture_snapshot/create_from_snapshot`、`save_to_file/load_from_file`。
   - CLI save/load 工作流：`--load-save`、`--save-out`、`--save-slot`、`--save-timestamp`。
   - `vn_previewd` 与 `preview protocol v1` 已落地，可供 editor/CI 复用。
   - `runtime` 与 `preview` 都已完成大文件拆分：当前分别按 `runtime_parse.c + runtime_cli.c + runtime_persist.c`、`preview_parse.c + preview_cli.c + preview_report.c` 收口。
...
2. 进行中:
   ...
   - x64/arm64 + Linux/Windows CI 矩阵保持全绿；最近一轮还额外修掉了 Linux native perf smoke 在 `-O2 -Werror` 下的构建告警回归。
